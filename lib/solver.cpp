            ...
            { // only consider nodes that haven't already been taken, and who have no remaining dependencies
                ...
                if (lower_bound >= best_cost)
                {
                    if (his_node != NULL)
                    {
                        ...
                        // node in the history table is better than the curr soln
...
bool solver::history_utilization(Key &key, int cost, int *lowerbound, bool *found, HistoryNode **entry)
{
    ...
    *lowerbound = content.lower_bound;
    ...
    { // TODO: thread stopping
        if (enable_threadstop && active_threads > 0 && target_ID != thread_id && !local_pools->out_of_work(target_ID))
...
bool solver::workload_request()
{
    ...
    if (!global_pool.empty())
    {
        ...
        if (!global_pool.empty())
        {
            problem_state = generate_solver_state(global_pool.back(), true);
    ...
    if (enable_workstealing)
    {
        ...
        while (true)
        {
            ...
            {
                HistoryNode *history_node = history_table.retrieve(new_node.history_key);
                history_node->active_threadID = thread_id;

                work_remaining[target] -= new_node.current_node_value;
                problem_state = generate_solver_state(new_node, false);
...
sop_state solver::generate_solver_state(path_node &subproblem, bool fromGlobalPool)
{
    ...
    for (int k = 1; k < size; k++)
    { // begin after the first, trivial, node
        int taken_node = subproblem.sequence[k];
    ...
    {
        state.history_key = subproblem.history_key;
    }
